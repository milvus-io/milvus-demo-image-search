...
import { FileCopyOutlined, AddCircleOutlineOutlined, RemoveCircleOutlineOutlined } from '@material-ui/icons';
    ...
    <form className={classes.formContainer}>
      <div className={classes.formItem}>
        <TextField
          ...
          value={form.primary}
          onBlur={() => { handleCheck(form.primary, "primary") }}
        ...
        />
        <FileCopyOutlined type="copy" className={classes.icon} onClick={() => { handleCopy(primaryRef.current.state.value) }} />
      </div>
            ...
            form.secondary.map((v, i) => (
              <li key={`${v}${i}`} className={`${classes.formItem}`}>
                <TextField
                  ...
                  value={v || ""}
                  onChange={(val) => { handleSecondaryChange(val, i) }}
                  className={classes.textField}
                  placeholder={dataTrans.second}
                  error={error.secondary}
                  helperText={error.secondary && `${dataTrans.second}${t('required')}`}
                />
                {i === 0
                  ? (<>
                    <FileCopyOutlined className={classes.icon} onClick={() => { handleCopy(v) }} />
                    <AddCircleOutlineOutlined className={classes.icon} onClick={handleAddPath}></AddCircleOutlineOutlined>
                  </>)
                  : (
                    <>
                      <FileCopyOutlined className={classes.icon} onClick={() => { handleCopy(v) }} />
                      <RemoveCircleOutlineOutlined className={classes.icon} onClick={() => { handleDeletePath(i) }}></RemoveCircleOutlineOutlined>
                    </>
                  )}
              </li>
            ))
